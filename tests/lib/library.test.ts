...
function meta(title: string, overrides: Partial<import('@/lib/papers').PaperMeta> = {}) {
  return {
    title,
...
async function placePaper(topic: string | null, slug: string, title: string, tags?: string[]) {
...
describe('slugify', () => {
  it('normalizes titles into safe folder names', async () => {
    ...
    expect(slugify('  Diffusion — Models: A Survey!! ')).toBe('diffusion-models-a-survey');
...
describe('paper CRUD on disk', () => {
  ...
  it('accepts an inbox capture into a topic (PDF + companion move together)', async () => {
    ...
    expect(fs.existsSync(path.join(lib.companionDir('nlp', 'fresh'), 'meta.json'))).toBe(true);
...
describe('index rebuild from disk', () => {
  it('indexes papers and finds them by title, tag, and full text', async () => {
    const lib = await placePaper('nlp', 'attention', 'Attention Is All You Need', ['transformers']);
    lib.rebuildIndex();
    expect(lib.searchIndex('attention').map((p) => p.slug)).toEqual(['attention']);
    expect(lib.searchIndex('transformers').map((p) => p.slug)).toEqual(['attention']);
  ...
  it('survives a moved file: rebuild reflects the new topic (disk wins)', async () => {
    const lib = await placePaper('nlp', 'attention', 'Attention Is All You Need');
    lib.rebuildIndex();
    ...
    fs.renameSync(lib.pdfPath('nlp', 'attention'), lib.pdfPath('classics', 'attention'));
    fs.renameSync(lib.companionDir('nlp', 'attention'), lib.companionDir('classics', 'attention'));
