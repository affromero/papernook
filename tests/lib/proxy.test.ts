...
describe("request proxy", async () => {
  it("keeps the production request limit unless a positive integer overrides it", async () => {
  ...
  it("allows the token-authenticated capture route without a session", async () => {
    const response = await proxy(new NextRequest("http://papernook.test/add"));
    expect(response.headers.get("x-middleware-next")).toBe("1");
  });

  it("uses a per-request nonce instead of allowing inline scripts", async () => {
    const first = await proxy(new NextRequest("http://papernook.test/login"));
    const second = await proxy(new NextRequest("http://papernook.test/login"));
  ...
  it("redirects unauthenticated pages to the picker without preserving query data", async () => {
    const response = await proxy(
  ...
  it("gates every hostname identically, with no host-selected weak mode", async () => {
    ...
    const rawPort = await proxy(
      new NextRequest("http://127.0.0.1:3000/settings", {
        headers: { host: "localhost" },
      }),
    );
    const publicName = await proxy(
  ...
  it("lets a share link through on its id alone", async () => {
    // The unguessable share id is the credential; a recipient has no account.
    const share = await proxy(
