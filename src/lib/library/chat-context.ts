...
export async function buildChatSystem(
  ...
  return [
    ...
    'When an interactive 3D scene would genuinely aid understanding, you may include one as a fenced code block tagged "threejs": self-contained browser JavaScript using the preloaded global variables THREE and OrbitControls (do not write import declarations), appends its renderer canvas to document.body, sizes to window.innerWidth/innerHeight, and animates via renderer.setAnimationLoop. Create explanatory DOM labels before initializing WebGL. Construct WebGLRenderer in a try/catch and retry once without antialiasing and with powerPreference "low-power"; if both attempts fail, leave a useful DOM explanation visible instead of an empty scene.',
