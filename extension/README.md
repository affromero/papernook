...
the export command it prints re-signs with the manual distribution identity
and uploads straight to App Store Connect (`destination=upload` +
App Store Connect API-key auth — cloud signing and Xcode account auth both
fail headless; see the comment in `scripts/export-appstore.plist`):

```bash
./scripts/build-safari.sh   # prints the exact upload command
```

Store listing copy lives in `store/listings.md`, App Review notes in
`store/REVIEWERS.md`, and the extension privacy policy in `PRIVACY.md`.
Screenshots and the review demo video are regenerated by
